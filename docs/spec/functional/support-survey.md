...
- Keep one cumulative successful action counter that never resets: successful replay scenario completions
...
## Click Repeater rules

- Counter tick:
   - Any successful replay scenario completion
- Survey window:
   - A modal window inside the extension popup
- Survey trigger:
   - Immediately after the successful replay scenario is complete, if the counter reached the threshold
